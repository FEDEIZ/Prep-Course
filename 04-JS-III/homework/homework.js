...
function dePalabrasAFrase(palabras) {
  ...
  for (let i = 0; i < palabras.length; i++) {
    ...
    if (i < palabras.length - 1) palabrasConcatenadas = palabrasConcatenadas + ' ';
  }
...
function mesesDelAño(array) {
  ...
  if (nuevoArray.includes('Enero', 'Noviembre', 'Marzo')) return nuevoArray;
...
function mayorACien(array) {
  ...
  arrayMayoresACien = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i] > 100) arrayMayoresACien.push(array[i]);
  }
...
function breakStatement(numero) {
  ...
  let i = 0;
  for (i; i < 10; i++) {
    arrayIteracion.push(numero + (i + 1) * 2);
    if (arrayIteracion[i] == i) break;
  }
  if (i < 10) return 'Se interrumpió la ejecución';
  else return arrayIteracion;
}
